...
class H5BHTMLWidget extends WP_Widget {
	...
	public function widget ($args, $widget) {
		...
		$id			= (isset($widget['id']) and strlen($widget['id'])) ? $widget['id'] : false;
		$element	= (isset($widget['element']) and strlen($widget['element'])) ? $widget['element'] : false;
		$classes	= (isset($widget['classes']) and strlen($widget['classes'])) ? $widget['classes'] : false;
		$content	= (isset($widget['content']) and strlen($widget['content'])) ? do_shortcode($widget['content']) : false;

		if ($element) {
			echo "<$element" . ($id ? " id=\"$id\"" : '') . ($classes ? " class=\"$classes\"" : '' ) . ">" . ($title ? "<h2>$title</h2>" : '') . "$content</$element>";
		}
		else {
			echo ($title ? "<h2>$title</h2>" : '') . $content;
		}
	}
